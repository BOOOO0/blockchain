...
- 위임된 PoS로 노드의 자격을 가진 사람들이 투표를 통해 상위 노드를 선출하고 상위 노드에 권한을 위임하여 권한을 위임 받은 소수의 노드만 참여하는 방식이다.

- 노드의 수가 적으므로 일반 PoS 트랜잭션을 빠르게 처리할 수 있지만 상위 노드 간 담합의 위험과 수가 적은 만큼 상위 노드를 향한 DDoS 공격에 취약한 단점이 있다.

## PoW (Proof of works)

- PoW는 특정한 해시를 찾아서 맞추는 과정을 반복하면서 작업에 참여를 증명하는 방식이다.

- 암호화폐의 채굴은 블록체인의 새로운 블록을 생성하고 그 대가로 암호화폐를 얻는 것이다.

- 새로운 블록을 생성하기 위해서는 새로운 거래내역(트랜잭션)을 생성하는 것이므로 노드에서 인정을 받아야 하고 그렇기 때문에 적절한 해시값을 찾아서 맞춰야 생성하고 새로운 블록이 추가된 트랜잭션이 인정된다.(?)

- 비트코인의 채굴 방식으로 이더리움도 PoW로 시작하였으나 2022년 merge 프로젝트로 PoS 알고리즘으로 업데이트 되었다.
